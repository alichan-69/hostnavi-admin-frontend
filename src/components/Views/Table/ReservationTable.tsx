...
export type TableReservation = {
  ...
  reserver: ReservationReservationResponseData['reserver']['name'];
  checkInTime: ReservationReservationResponseData['checkInTime'];
  checkOutTime: ReservationReservationResponseData['checkOutTime'];
  guestNumber: ReservationReservationResponseData['guestNumber'];
  fee: ReservationReservationResponseData['fee'];
};
...
const ReservationTable = function render() {
  ...
  const handleChangeSearchedText: TextFieldOnChange = (event) => {
    setSearchedText(String(event.target.value));

    searchedColumn?.setFilterValue(String(event.target.value));
  };
  ...
  const getAllReservationsWrapper = async () => {
    ...
    const responseReservations = await getAllReservations(userId);
    setReservations(
  ...
  useEffect(() => {
    ...
    loadingRequest(getAllReservationsWrapper(), '取得中', '取得完了', '取得中にエラーが発生しました');
